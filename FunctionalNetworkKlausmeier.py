...
#         # timing update (leve, não pesa muito)
...
#         if i % 5000 == 0 and i > 0:
#             eta = (nsteps - i) * avg_step_time
#             print(f"  step {i}/{nsteps} | ETA: {eta/60:.2f} min")

#     total_time = time.time() - t0
#     print(f"Finished a = {a:.3f} | time = {total_time/60:.2f} min")

#     return np.array(functional_snapshots), Nfield
...
# a_max = 0.32
# N_a = 100
# a_values = np.linspace(a_max, a_min, N_a)

# # RUN + SAVE
# with h5py.File("functional_networks_plus.h5", "w") as f:
...
#         print(f"\n[{i+1}/{N_a}] Running a = {a:.3f}")

#         snapshots, final_state = run_simulation(a)
